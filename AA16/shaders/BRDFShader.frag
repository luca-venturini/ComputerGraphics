...
float f(float p, float q, float sigma){
	...
	float theta_i = acos(p);
	float theta_r = acos(q);
	float alpha = max(theta_i, theta_r);
	float beta = min(theta_i, theta_r);
	float B = 0.45 * ( pow(sigma, 2)/(pow(sigma, 2) + .09 ) );
...
vec3 Phong_Specular_BRDF(vec3 L, vec3 N, vec3 V, vec3 C, float gamma)  {
	...
	return C * pow(clamp(dot(V,(-reflect(L, N))), 0, 1), gamma);
}
