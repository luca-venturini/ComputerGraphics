...
vec3 direct_light_dir(vec3 pos) {
	...
	return gubo.lightDir; //vec3(0.0f, 0.0f, 1.0f);
}

vec3 direct_light_color(vec3 pos) {
	// Directional light color
	return gubo.lightColor; //vec3(0.5f, 1.0f, 0.5f);
}

vec3 point_light_dir(vec3 pos) {
	// Point light direction
	return normalize(gubo.lightPos - pos); // vec3(0.0f, 0.0f, 1.0f);
}
...
vec3 spot_light_color(vec3 pos) {
	// Spot light color
	return gubo.lightColor * pow( (gubo.coneInOutDecayExp.z/length(gubo.lightPos - pos)), gubo.coneInOutDecayExp.w ) * clamp( ( dot(normalize(gubo.lightPos - pos), gubo.lightDir) - gubo.coneInOutDecayExp.x ) / (gubo.coneInOutDecayExp.y - gubo.coneInOutDecayExp.x), 0, 1 );
}
